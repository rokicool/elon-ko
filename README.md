...
| **`elon-ko-agents`** | **10 specialist agents** + **11 skills**: `reqguru`, `drpe`, `leaddev`, `middev`, `validator`, `purecode`, `docworm`, `hr`, `wrapper`, `debugger` (plus the `elon` orchestrator protocol). |
...
```bash
curl -fsSL https://raw.githubusercontent.com/rokicool/elon-ko/main/elon_ko.sh | OMP_AGENT_REF=v2.7.1 bash
```
...
omp plugin install github:rokicool/elon-ko#v2.7.1
# local dev / linking:
...
Both are fetched from the repo at the **same ref as Plugin A** (`OMP_AGENT_REF`, default `v2.7.1`), so the deployed guidance always matches the installed gate version.
...
> **Release timing.** The `RULES.md`→rule move and the `AGENTS.md` coherence edit ship as of **v2.4.0** — the **stable** default install (`#v2.7.1`) fetches Plugin A *and* the scaffold files from the published `v2.7.1` tag, so the gate carries the `ro-orchestrator-invariant` rule and the deployed `AGENTS.md` no longer references `RULES.md`. Older tagged installs (`#v2.3.1` and earlier) predate these changes and do not carry them.
...
(`reqguru`, `drpe`, `leaddev`, `validator`, `purecode`, `docworm`, `hr`, `wrapper`, `debugger`), write only
...
| `pi/task` | `middev`, `reqguru`, `validator`, `purecode`, `debugger` | Strong general (implementation, clarification, auditing). |
...
modelRoles:
  ...
  task: anthropic/claude-sonnet-4-5      # middev, reqguru, validator, purecode, debugger
