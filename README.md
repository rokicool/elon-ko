...
```bash
curl -fsSL https://raw.githubusercontent.com/rokicool/elon-ko/main/elon_ko.sh | OMP_AGENT_REF=v2.8.0 bash
```
...
omp plugin install github:rokicool/elon-ko#v2.8.0
# local dev / linking:
...
Both are fetched from the repo at the **same ref as Plugin A** (`OMP_AGENT_REF`, default `v2.8.0`), so the deployed guidance always matches the installed gate version.
...
> **Release timing.** The `RULES.md`→rule move and the `AGENTS.md` coherence edit ship as of **v2.4.0** — the **stable** default install (`#v2.8.0`) fetches Plugin A *and* the scaffold files from the published `v2.8.0` tag, so the gate carries the `ro-orchestrator-invariant` rule and the deployed `AGENTS.md` no longer references `RULES.md`. Older tagged installs (`#v2.3.1` and earlier) predate these changes and do not carry them.
