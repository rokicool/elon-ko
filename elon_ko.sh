...
if [ -n "$TAG" ]; then
  ...
  MODE="stable"
  REF="${OMP_AGENT_REF:-v2.1.0}"        # static tag: avoids store ref-drift + network deps; OMP_AGENT_REF overrides for dev
fi
...
if [ "$MODE" = "pre-release" ]; then
  ...
  # (its name depends on the ref — e.g. v2.1.0 → elon-ko-2.1.0).
